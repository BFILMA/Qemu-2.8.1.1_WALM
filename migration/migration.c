...
int iter = 0;
int64_t downtime_page = 0,page_count = 0;
int eos_flag = 0;
//
...
void qemu_start_incoming_migration(const char *uri, Error **errp)
{
    ...
    if (!strcmp(uri, "defer")) {
        ...
        //Jon: start the itmap_thread here
        //creating bitmap on the destination for postcopyFT
        QemuThread bitmap_thread;
        qemu_thread_create(&bitmap_thread,"bitmap_create",create_bitmap_thread,NULL,QEMU_THREAD_DETACHED);
#ifdef CONFIG_RDMA
...
static int postcopy_start(MigrationState *ms, bool *old_vm_running)
{
    ...
    *old_vm_running = runstate_is_running();
     ...
     */
#ifdef enable_rate_limiting	
    qemu_file_set_rate_limit(ms->to_dst_file, INT64_MAX); //TODO ratelimiting
#endif
    ...
    if (s->state == MIGRATION_STATUS_ACTIVE) {
        ...
        *old_vm_running = runstate_is_running();
        ...
        if (!ret) {
			...
			{
#ifdef enable_rate_limiting				
                qemu_file_set_rate_limit(s->to_dst_file, INT64_MAX); //TODO ratelimiting
#endif
...
#ifdef enable_rate_limiting
		printf("hello1 \n");
        if (!qemu_file_rate_limit(s->to_dst_file)) { //TODO ratelimiting
#endif
			...
			//	if(!(((uint64_t)(pending_size/4096LLU)) <= 5000LLU )||(iter <= 50)){
				iter++;
            //printf
			if (pending_size && pending_size >= max_size) {
				...
				printf("Iteration %d pages %d \n",iter,page_count);
			} else {
				trace_migration_thread_low_pending(pending_size);
				migration_completion(s, current_active_state,
						&old_vm_running, &start_time);

				printf("Iteration %d downtime pages %d \n",iter,downtime_page);
				break;
			}
#ifdef enable_rate_limiting			
	//	}  //TODO ratelimiting
...
#ifdef enable_rate_limiting		
            qemu_file_reset_rate_limit(s->to_dst_file); //TODO ratelimiting
#endif
...
#else
//		g_usleep((initial_time + BUFFER_DELAY - current_time)*1000); //TODO uncomment 
...
#ifdef enable_rate_limiting	
    qemu_file_set_rate_limit(s->to_dst_file,s->parameters.max_bandwidth / XFER_LIMIT_RATIO); //TODO ratelimiting
#endif 
